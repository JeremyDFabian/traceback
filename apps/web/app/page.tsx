...
const stages = ["Preparing your notebook page", "Finding ideas and labels", "Mapping arrows and study markers", "Linking your study surface"];
...
function WhatIsTraceback() {
  ...
  return <section id="what-it-is" ref={sectionRef} className={`what-is-traceback ${isVisible ? "is-visible" : ""}`} aria-labelledby="what-is-traceback-title"><p className="eyebrow">What is Traceback?</p><h2 id="what-is-traceback-title"><span>Your handwritten page should be</span><em>a living concept map—not a forgotten PDF.</em><span>Traceback connects it to the material behind it.</span></h2><p className="what-is-note">It reads the ideas, diagrams, arrows, and study marks on your page, then keeps every connection clickable alongside the lecture slides that explain it.</p></section>;
}
...
function HowItWorks() {
  ...
  return <section id="how-it-works" ref={sectionRef} className={`how-it-works ${isVisible ? "is-visible" : ""}`} aria-labelledby="how-it-works-title"><p className="eyebrow" id="how-it-works-title">How Traceback works</p><div className="how-it-works-grid"><article className="process-card capture-card"><span>Step 01</span><div className="process-visual camera-visual"><i /><b>⌁</b></div><h3>capture your context</h3><p>Photograph a handwritten page and add the lecture slides it belongs with.</p></article><article className="process-card confirm-card"><span>Step 02</span><div className="process-visual region-visual"><i>mitochondria → ATP</i><i>ATP ★</i><i>cellular respiration ?</i></div><h3>map how you think</h3><p>Traceback recognizes concepts, diagrams, arrows, and study marks—then makes those relationships editable.</p></article><article className="process-card trace-card"><span>Step 03</span><div className="process-visual slide-visual"><small>LECTURE 03</small><b>How cells make<br /><mark>usable energy</mark></b><i>Slide 7 · matched</i></div><h3>trace, then recall</h3><p>Jump from a notebook idea to its source slide, then turn stars and questions into a spaced study queue.</p></article></div></section>;
}
...
export default function Page() {
  ...
  return <main className="app-shell">
    ...
    {screen === "setup" && <><section className="landing"><div className="hero-copy"><p className="eyebrow">From notebook to laptop</p><h1>Keep your handwritten<br /><em>thinking in motion.</em></h1><p className="hero-description">Traceback turns your notes, sketches, and arrows into a clickable study map—connected to the lecture slides that give every idea its context.</p><div className="trust-row"><span>✦ Semantic canvas mapping</span><span>◌ Slide sync &amp; study cues</span></div><div className="opening-notebook" aria-hidden="true"><div className="notebook-spine" /><div className="notebook-cover"><span>trace</span></div><div className="notebook-page left"><i /><i /><i /></div><div className="notebook-page right"><i /><i /><i /></div><div className="notebook-spark one">✦</div><div className="notebook-spark two">✦</div></div></div><div className="setup-card"><div className="setup-heading"><span className="step-number">01</span><div><p className="eyebrow">Start a study session</p><h2>Bring your materials</h2></div></div><UploadField label="Upload lecture slides" detail="PDF · Your reference material" accept="application/pdf" file={deck} onChange={(event) => setDeck(event.target.files?.[0])} /><UploadField label="Photograph a notebook page" detail="JPG, PNG, or take a photo" accept="image/*" capture file={notebook} onChange={selectNotebook} /><button className="primary-button" disabled={!deck || !notebook} onClick={beginAnalysis}>Map my notes <span>→</span></button><p className="privacy-note">Your files are used only for this study session.</p></div></section><WhatIsTraceback /><HowItWorks /></>}
